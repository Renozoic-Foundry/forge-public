...
| v4.1.0 | MINOR | Additive commands + process refinements (released 2026-08-09, ADR-675 — 32-spec window; `/test` Usage change was the MINOR anchor). |
| v4.1.1 | PATCH | Process-kit publication apply + reference-closure (released 2026-08-09, ADR-708): 47 process-kit guides now ship and resolve for public consumers; prose re-points only, no versioned-surface change. |
| v5.0.0 | MAJOR | **NanoClaw removal — final wind-down release** (Spec 654, ADR-708 window; Spec 708 cut). The public `/nanoclaw` and `/configure-nanoclaw` commands are **removed** (the NanoClaw subsystem was intentionally retired as non-production; hardware-authenticated gates / PAL deprecated with it). A one-time, non-generalized deprecation waiver covers these two commands (release-policy.md § Deprecation waivers). **Migration**: no action for projects that never used NanoClaw; projects referencing `/nanoclaw` or `/configure-nanoclaw` must remove those references — the subsystem is gone and `gate.provider: prompt` is the only supported gate mode. Also ships in this window: guard/detector hardening (676/677/681/661), doctrine_gen CRLF-preservation (707), compose destination-tree closure (637), and the model/effort measurement corrections (679/680). |
