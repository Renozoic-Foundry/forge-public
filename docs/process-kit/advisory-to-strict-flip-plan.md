...
A gate that ships advisory and stays advisory forever is equivalent to no gate. This plan makes the flip predictable, owner-attributed, and reversible.
...
The mechanical criteria are necessary but not sufficient. The subjective layer preserves operator agency and prevents premature flips driven solely by mechanical signals.
...
**Spec 327 flip**: the project owner (same as Spec 327 Approver).

**Spec 330 flip**: the project owner (same).
...
A flip-back is a normal operational event, not a failure of the gate. The advisory-mode default exists to make this reversible.
...
| Spec 411 | `bash ${CLAUDE_PLUGIN_ROOT:-.}/.forge/lib/two-list-bypass-detect.sh` | n/a | 0 | 4 prose-ignores, 0 block-ignores, 0 coordinated bypass (criterion 4) |
