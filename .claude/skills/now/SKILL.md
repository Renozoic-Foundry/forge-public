...
```
On FORGE-self (no operator-installed managed settings) the expected value is `hooks-only`.
Do not add follow-up prompts or detail — the one-line posture is the entire output of this step.

**Autopilot-envelope posture (Spec 531)**: when AGENTS.md contains a `forge.autopilot:` block,
append ` · autopilot envelope: scheduled <off|on>` to the Authority line (read the
`scheduled.enabled` value; read-only — never validate here, /close owns the gate). When the
...
### [mechanical] Step 1b — Unclosed-spec cap + file-overlap surfacing (Spec 498)

Deferred-close chaining (`implement -> implement_next`) accumulates specs at `implemented`
without an intervening `/close`. This step surfaces that pile-up so it cannot grow silently and
mechanically flags the riskiest case — a later spec building on an earlier spec's **unreviewed**
output (declared-file overlap). Read the cap from `forge.now.unclosed_spec_cap` in AGENTS.md
(default: **3** if absent).

1. **List implemented-but-unclosed specs**: reuse the Step 1 set (specs at status `implemented`,
   not `closed`). Let `U` = that set and `count = |U|`.
   - If `count == 0`: skip this step silently.
   - Else emit exactly one line with the count AND the IDs (Req 6 — never count-only here):
     `Unclosed (implemented, awaiting /close): <count> — <comma-separated spec IDs>.`
2. **Cap warning**: if `count > cap`, emit a warning line (never suppressed, even in lean mode):
   `⚠ Unclosed-spec cap exceeded: <count> implemented specs await /close (cap = <cap>). Deferred-close chaining stacks unreviewed specs on a moving baseline — clear the validation queue with /close before chaining further (Spec 498).`
3. **File-overlap surfacing (chained hazard — AC7, the case that must be mechanical, not only the count)**:
   for each pair of specs in `U`, read each spec's `## Implementation Summary` `Changed files` list and
   intersect the declared paths. If two unclosed specs share ANY changed-file path, emit one line per
   overlapping pair:
   `⚠ File overlap among unclosed specs: Spec <A> and Spec <B> both touch <path(s)> — <B> may build on <A>'s unreviewed output. Validate /close <A> before relying on <B>.`
   - If no pair overlaps: silent on this sub-point.

This step is read-only and advisory — it never blocks. It complements Step 1 (which offers the
...
8c. **Process-kit external-source freshness check** (Spec 278): Read `forge.process_kit.freshness_threshold_days` from AGENTS.md (default: **180** if absent or unset). Scan `.md` files under `docs/process-kit/` AND `template/docs/process-kit/` for a `<!-- Last verified: YYYY-MM-DD against <source-url> -->` marker within the first 10 lines.
   ...
   - **Public-doc stamp check (Spec 509)**: additionally run `bash ${CLAUDE_PLUGIN_ROOT:-.}/.forge/lib/freshness.sh check` (skip silently if the helper is absent). Each emitted line is a public doc whose Spec 278 marker was stamped STALE at `/close` because a closing spec changed a documented surface (the surface→doc resolution is the Spec 511 canonical mapping, owned by the helper — do not re-inline it here). If any lines emitted, append to the same report:
     ...
     ```
     Re-verify by refreshing the doc and updating its marker (generated docs clear the stamp on regeneration — see the helper header for the generated-doc interaction).
