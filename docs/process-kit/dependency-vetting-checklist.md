...
Use this checklist when reviewing new or significantly changed dependencies flagged by the `/implement` dependency-confirmation gate (Step 6a0) via its `DEPENDENCY_REVIEW_REQUIRED` signal. (The standalone dependency-audit command was retired in v4.0.0 — Specs 587, 627.)
...
- `/implement` — scans for dependency changes inline (Step 6a0) and emits `DEPENDENCY_REVIEW_REQUIRED` when review is needed
