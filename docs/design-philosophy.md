...
Human authority is inviolable. FORGE has iteratively hardened its authorization enforcement — starting from prose guidance and advancing to mechanical runtime hooks as real incidents revealed weaker controls were insufficient. No agent confirms on your behalf. No command auto-chains past a gate that hasn't received explicit human approval — at every autonomy level, L0 through L4. Every signal draft, every spec, every close requires explicit operator approval. The developer is the CEO; the AI is staff.
...
| **L3** | Trusted Autonomy | Chains implement/close cycles without per-gate pauses | You invoke `/close` and approve every push — the operator gate never delegates |
| **L4** | Full Autonomy | Self-directed with evolve loop; kill switch and budget ceilings are hard stops | You set objectives, review outcomes; `/close` stays yours |

The default is L1 because trust must be earned through evidence. Moving to L2 requires that evidence gates consistently pass without human intervention. Moving to L3 requires a track record — consistent gate pass-rates at L2 plus operator confidence in the evidence trail. What L3 buys is chain continuation: the agent keeps working through gates without a per-gate "continue?" pause. What it does not buy is authorization — `/close` and `git push` remain operator actions at every level, L3 and L4 included. L4 requires both strong evidence gates and operator confidence that the Evolve Loop is producing genuine improvements.
