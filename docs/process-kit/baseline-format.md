...
> **RETIRED SURFACE (Spec 558, v4.0.0)**: this guide documents the Copier-based
> consumption path, which was deleted in v4.0.0. It remains accurate for classic
> consumers on <=v3.x releases only. See docs/process-kit/migration-decision-guide.md
> for the current path and the `--to-plugin` on-ramp.
