...
> **Cap-fire is an exception, not the normal path (Spec 649 / ADR-649).** Until 2026-08-07 the exit
> predicate required on-disk `Status: closed`, which `/implement` never writes and Priority 1 forbids
> it from writing — so *every* unattended run burned to the cap and this runbook described the
> ordinary flow. That is fixed: the predicate is now `Status: implemented`, which the command reaches
> on its own. A cap-fire today means something actually went wrong. Treat it as a signal, not routine.
...
1. **Re-engage manually** — drop out of `--goal-mode` and finish the remaining ACs interactively. The
   ...
   `Status: implemented`, gate-emission log) are still unmet and which gates are missing.
