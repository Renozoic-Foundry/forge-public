# FORGE NanoClaw Configuration Wizard — PowerShell wrapper
# Usage: .\forge-configure-nanoclaw.ps1 [--step N] [--check-only]
#
# Requires Git Bash (bash.exe) — included with Git for Windows.

$ErrorActionPreference = "Stop"
$ScriptDir = Split-Path -Parent $MyInvocation.MyCommand.Path
$BashScript = Join-Path $ScriptDir "forge-configure-nanoclaw.sh"
...
$TempFile = Join-Path $env:TEMP "forge-configure-nanoclaw-$PID.sh"
