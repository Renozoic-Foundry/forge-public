#!/usr/bin/env bash
# FORGE Role Permission Check — PreToolUse hook (schema migrated Spec 499)
...
#
# Schema (Spec 499): PreToolUse blocks via the documented
...
# 2026-06-24), not the documented PreToolUse contract. Allow paths emit nothing
# (exit 0 = defer to normal permission flow). deny is a HARD BLOCK — it raises no
# in-session permission dialog; to write, the operator runs the action manually.
...
if [ "$READ_ONLY" = "true" ]; then
  jq -nc --arg r "Role '${ROLE_NAME}' is read-only — Write/Edit blocked by FORGE role enforcement" \
    '{hookSpecificOutput:{hookEventName:"PreToolUse",permissionDecision:"deny",permissionDecisionReason:$r}}'
  exit 0
fi

# Default: allow (defer to normal permission flow)
exit 0
