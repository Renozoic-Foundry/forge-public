...
def get_backlog_rows(project_root: Optional[Path] = None) -> list[dict]:
    ...
    Output order:
      ...
      3. status=draft or approved, sorted by score desc then spec_id (numeric rank assigned)
    ...
    # Ranked pool: draft + approved (Spec 467 — approved specs previously
    # vanished from the backlog because this pool only pulled "draft").
    drafts = sorted(
