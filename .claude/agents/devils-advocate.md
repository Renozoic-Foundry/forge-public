...
## Pre-review surface-liveness check (Spec 450)

If the spec's `Last updated:` frontmatter date is more than 7 days before today: before reviewing the spec body, verify every file path and code symbol named in the spec's `## Implementation Summary` exists at HEAD (entries annotated `(new)` are files the spec will create — do not flag them). If any unannotated references are obsolete, flag scope obsolescence as your primary finding. Report it inside your existing output schema — a `findings[]` entry with `domain: "scope-obsolescence"`; do not add new JSON fields. Specs updated within the last 7 days skip this check.
