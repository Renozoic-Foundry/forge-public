...
[![Validate Template](https://github.com/bwcarty/forge-public/actions/workflows/validate-template.yml/badge.svg)](https://github.com/bwcarty/forge-public/actions/workflows/validate-template.yml)

A framework for AI-assisted, spec-driven software delivery with built-in learning and governance — from rapid prototyping to safety-critical firmware.
...
4. **Spec Kit** — Every change has a versioned spec with objective, scope, requirements, acceptance criteria, test plan, and revision log. Specs are rebuild guides — the codebase can be reconstructed from specs alone. Specs also serve as **context anchors**: living documents that persist decision context across AI sessions, team changes, and time. Martin Fowler's writing on context anchoring (2026) independently validated this pattern — FORGE has practiced it from the start.
...
FORGE uses [Model Context Protocol](https://modelcontextprotocol.io/) servers to ensure agents work from current documentation rather than stale training data. This is especially critical for Lane B (safety-critical) projects where regulatory accuracy has no error budget.
...
**Lane B (compliance engine, per profile):**
- **Ansvar EU Regulations MCP** — 49 EU regulations with daily EUR-Lex sync. Raw regulation text, zero LLM summarization.
- **Grounded Docs MCP** — Self-hosted indexing of purchased standard PDFs (IEC, ISO, FDA, internal docs). Documents never leave your machine.

Each compliance profile declares its MCP servers, gate rules, and artifact templates. MCP servers are declared in `.mcp.json` at the project root. Lane B projects enforce a documentation accuracy rule in CLAUDE.md: agents must query the MCP server for current authoritative text and cite specific clauses — never rely on training data for compliance requirements.
...
## Compliance Engine — Regulation-Agnostic Lane B

FORGE's Lane B provides a compliance engine that can enforce any regulatory or standards framework, as long as agents have access to authoritative, current documentation via MCP servers. Specific regulations are plugged in as **compliance profiles** that define gate rules, artifact templates, and documentation sources.

### Example Compliance Profiles

| Profile | Framework | Use Case |
|---------|-----------|----------|
| `eu-machinery` | EU 2023/1230 + IEC 61508 + IEC 62443 | PLC/HMI firmware for EU market (reference profile) |
| `automotive` | ISO 26262 + ASPICE | ADAS, ECU, powertrain software |
| `medical` | FDA 21 CFR Part 11 + IEC 62304 | Medical device firmware |
| `fintech` | SOC 2 + PCI DSS | Financial services applications |
| `internal` | Corporate engineering standards | Any organization's internal compliance |

### North Star: EU Machinery Regulation 2023/1230

The `eu-machinery` profile is the reference implementation, driven by the **January 20, 2027** hard deadline. This regulation introduces software as a safety component, cybersecurity in CE marking, 10-year firmware patch obligations, and mandatory third-party assessment for AI-driven safety functions. MISRA C:2025 explicitly states AI-generated code must meet identical verification standards as human-written code.

FORGE's evidence-gated lifecycle maps directly to what certification bodies audit: documented processes, bidirectional traceability, change impact analysis, and human review records. The development *process* is what gets certified, not the authorship method.

See [docs/roadmap.md](docs/roadmap.md) for the phased plan to deliver Lane B capabilities.

## Reference Implementation

FORGE was built using its own methodology — 200 specs across 47 sessions, validating the full lifecycle from draft through closure. The entire development history (specs, session logs, signals, ADRs) is preserved in this repository as evidence of the methodology in practice.

## Important Disclaimer

**FORGE is a development process framework, not a certification authority.**

FORGE assists with documentation, traceability, and evidence capture for software and firmware development. Compliance profiles and their associated MCP documentation servers are tools that help qualified professionals organize their compliance work — they do not constitute legal, regulatory, or safety engineering advice.

- Compliance artifacts generated by FORGE (traceability matrices, V&V reports, compliance cases) are **aids for qualified professionals**, not substitutes for independent verification
- **No warranty of compliance** is expressed or implied — using FORGE does not guarantee that your product meets any regulatory standard
- Ingested regulatory material (via MCP servers or local document indexing) must be **verified by a qualified human** against authoritative sources before being relied upon for compliance decisions
- The developer, their organization, and their qualified auditors bear **full responsibility** for regulatory compliance, safety validation, and fitness for purpose of all shipped products
- FORGE does not replace certified auditors, functional safety engineers, domain experts, or legal counsel

When activating a compliance profile, FORGE requires a **Compliance Profile Verification** sign-off confirming that a qualified human has verified the documentation sources, standard versions, and scope applicability. This verification must be reviewed periodically (default: quarterly).
