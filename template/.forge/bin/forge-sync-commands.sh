...
# --- Spec 491: skill-form names are NOT emitted as .claude/commands mirrors ---
# The invocation-policy.yaml manifest (skills_model_invokable + skills_explicit) lists the
# names that live as .claude/skills/<name>/SKILL.md only (generated by forge-sync-skills.sh).
# This generator must skip those names so it neither emits nor checks a stale command form.
# Read the skill sets from the policy file co-located with the canonical command dir.
is_skill_form() {
  local name="$1"
  local policy_file="${CANONICAL_DIR}/invocation-policy.yaml"
  [[ -f "$policy_file" ]] || return 1
  local key line names
  for key in skills_model_invokable skills_explicit; do
    line="$(grep -E "^${key}:" "$policy_file" | head -1)"
    [[ -n "$line" ]] || continue
    names="${line#*[}"
    names="${names%]*}"
    # Word-boundary match against the comma/space-separated list.
...
if $CHECK_MODE; then
  ...
  for src_file in "$CANONICAL_DIR"/*.md "$CANONICAL_DIR"/*.md.jinja; do
    ...
    check_cmd_name="${src_base%.md.jinja}"
    check_cmd_name="${check_cmd_name%.md}"
    if is_skill_form "$check_cmd_name"; then
      continue
    fi
  ...
  for dst_file in "${CHECK_TARGET_DIR}/"*.md "${CHECK_TARGET_DIR}/"*.md.jinja; do
    ...
    orphan_cmd_name="${dst_base%.md.jinja}"
    orphan_cmd_name="${orphan_cmd_name%.md}"
    if is_skill_form "$orphan_cmd_name"; then
      echo "DRIFT: $dst_base — skill-form name has a stale command mirror (Spec 491: must be .claude/skills only)"
      DRIFT_COUNT=$((DRIFT_COUNT + 1))
      continue
    fi
...
for agent in "${AGENTS[@]}"; do
  ...
  for src_file in "$CANONICAL_DIR"/*.md "$CANONICAL_DIR"/*.md.jinja; do
    ...
    # Spec 491: skip skill-form names for the claude-code surface — they ship as
    # .claude/skills/<name>/SKILL.md (generated by forge-sync-skills.sh), not as a
    # .claude/commands mirror. Other runtimes (cursor/copilot/cline) still receive the
    # command form from canonical, so the skip is claude-code-only.
    if [[ "$agent" == "claude-code" ]]; then
      cmd_name="${src_base%.md.jinja}"
      cmd_name="${cmd_name%.md}"
      if is_skill_form "$cmd_name"; then
        continue
      fi
    fi
