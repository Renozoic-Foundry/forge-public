...
8. **Strategic fit evaluation** (Spec 110, extended by Spec 363, sentinel-aware via Spec 382):
   ...
   b. Read AGENTS.md — check for `forge.strategic_scope` config under `## Project Context`. **Use the yaml-aware helper at `${CLAUDE_PLUGIN_ROOT:-.}/.forge/lib/strategic-scope.py`** to read the value (NOT regex on raw text — fragile against block-scalar variants per Spec 382 AC6):
      ```bash
      ...
      strategic_scope_value=$(${CLAUDE_PLUGIN_ROOT:-.}/.forge/bin/forge-py ${CLAUDE_PLUGIN_ROOT:-.}/.forge/lib/strategic-scope.py read AGENTS.md 2>"$_scope_err")
      strategic_scope_rc=$?
      ...
      if [ "$strategic_scope_rc" -ne 0 ] && [ -n "$strategic_scope_stderr" ]; then
        ...
        echo "      → Helper: ${CLAUDE_PLUGIN_ROOT:-.}/.forge/lib/strategic-scope.py | Strategic-fit eval is being skipped due to FAILURE, not configuration. Do NOT treat the backlog as fully vetted — investigate (missing PyYAML, Python <3.10, file/encoding error, or helper bug) before trusting the ranking."
      ...
      ```
   b1. **SKIP-FOR-NOW sentinel check (Spec 382)**: if the value equals literal `SKIP-FOR-NOW` (verifiable via `${CLAUDE_PLUGIN_ROOT:-.}/.forge/bin/forge-py ${CLAUDE_PLUGIN_ROOT:-.}/.forge/lib/strategic-scope.py is-sentinel AGENTS.md`), emit the one-line warning and skip the strategic-fit eval entirely:
      ```
...
13. Ask for confirmation, then:
   - **Write-side mode check (Spec 399)**: Before any canonical-table-row write, run `${CLAUDE_PLUGIN_ROOT:-.}/.forge/bin/forge-py ${CLAUDE_PLUGIN_ROOT:-.}/.forge/lib/derived_state.py --skip-canonical-write`. Read stdout: if `skip`, do NOT write status corrections or manual-override resolutions to `docs/backlog.md` — the canonical artifact is renderer-owned in split-file mode and the writes happen via the renderer in step 261-269 below (status auto-corrections become spec-frontmatter edits, which the renderer picks up on next render). If stdout is `proceed`, perform the canonical writes (Phase 1 dual-write behavior). If the helper exits nonzero, abort the canonical-write step entirely and surface stderr — do NOT default to either behavior.
   ...
   - **Spec 254 — Regenerate canonical table** (mode=generated only): After spec-file edits land, invoke `${CLAUDE_PLUGIN_ROOT:-.}/.forge/bin/forge-py ${CLAUDE_PLUGIN_ROOT:-.}/.forge/lib/render_backlog.py --output docs/backlog.md` to refresh the generated table portion of the canonical artifact. Skip silently if the project is on mode=delete or pre-Spec-254 monolithic format (no sentinel markers in docs/backlog.md).
   - **Spec 398 — Split-file rendering** (when `docs/.generated/` directory exists): renderers always write to `docs/.generated/<artifact>.md` (no mode-detection on curated parents). After all 3 renders, invoke the assembler for interactive operator stdout view. Skip the Spec 254 mode=generated path above (split-file supersedes it for migrated projects):
     ...
     ${CLAUDE_PLUGIN_ROOT:-.}/.forge/bin/forge-py ${CLAUDE_PLUGIN_ROOT:-.}/.forge/lib/render_changelog.py --mode=split-file
     ${CLAUDE_PLUGIN_ROOT:-.}/.forge/bin/forge-py ${CLAUDE_PLUGIN_ROOT:-.}/.forge/lib/render_spec_index.py --mode=split-file
     ${CLAUDE_PLUGIN_ROOT:-.}/.forge/bin/forge-py ${CLAUDE_PLUGIN_ROOT:-.}/.forge/lib/assemble_view.py docs/backlog.md
     ```
