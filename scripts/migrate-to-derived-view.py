...
def main(argv: list[str] | None = None) -> int:
    ...
    # Spec 427 Req 10 / AC 9: detect copier-post-task context. When running
    # inside `/forge stoke`'s copier-direct invocation, the parent process sets
    # FORGE_COPIER_POST_TASK=1. In that case, this script must NOT re-check the
    # dirty-tree state — stoke already ran its own pre-apply guard and copier
    # has staged its own changes in the working tree. The detection is exposed
    # for symmetry with copier.yml _tasks (which performs the actual skip).
    import os as _os
    _in_copier_post_task = bool(_os.environ.get("FORGE_COPIER_POST_TASK"))
    if _in_copier_post_task:
        print(
            "[Spec 427] Detected copier-post-task context (FORGE_COPIER_POST_TASK=1) — "
            "dirty-tree pre-checks deferred to stoke parent.",
            file=sys.stderr,
        )
