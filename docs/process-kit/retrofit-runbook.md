...
| `reorganize` | `git mv` process data → `.forge/project/`; writes `forge.paths` + `ownership.yaml` | operator confirms move list; verify after with `.forge/lib/check_doc_links.py` + `forge-doctor` (D-PATHS) | snapshot / `git mv` back |
